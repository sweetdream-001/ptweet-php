...
// Privacy Page
	...
	if (not_empty($ad_data)) {
		...
		} else if ($ad_data['status'] == "inactive"){
			cl_update_ad_data($ad_id, array(
				"status" => "active"
			));
...
// 	));
    ...
    $data['status']    = 200;
    $categories = $db->rawQuery("SELECT a.*, (SELECT name FROM cl_categories b WHERE b.id = a.parent_id) as parent_name FROM cl_categories as a WHERE parent_id IS NOT NULL ORDER BY parent_name ASC, a.name ASC");
    // foreach($categories as $k => $cat){
    //     $categories[$k]['childs'] = $db->rawQuery("SELECT * FROM `cl_categories` where parent_id = " . $cat['id']);
    // }
