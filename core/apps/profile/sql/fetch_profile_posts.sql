...
SELECT 
    ...
    posts.`type`, 
    posts.`user_id`,
    cl_categories.name as cat_name, 
    cl_categories.id as cat_id
FROM `<?php echo($data['t_posts']); ?>` posts

INNER JOIN `<?php echo($data['t_pubs']); ?>` pubs 
    ON posts.`publication_id` = pubs.`id`

LEFT JOIN cl_categories 
    ON pubs.`category_id` = cl_categories.id
